...
int CreateOnHardDisk()
{
    ...
    while(temp -> FileType != 0)
    {
        ...
        fWrite = creat(Fname,0777);

        strcpy(Data,temp -> Buffer);
        write(fWrite,Data,strlen(Data));
...
void Exit()
{
    ...
    printf("Do you want to save all files on HardDisk\n");
    printf("Y/N\n");
    scanf("%c",&ch);

    if((ch == 'Y') || ch == 'y')
    {
        ret = CreateOnHardDisk();
        if(ret == -1)
        {
            printf("There are no files to save\n");
        }
        else if(ret == 0)
        {
            printf("Files successfully saved on HardDisk\n");
        }
    }
...
int main()
{
    ...
    while(1)
    {
        ...
        if(count == 1)
        {
            ...
            {
                Exit();
                printf("Terminating the Siddhant's Virtual File System\n");
                break;
            }
            else
            {
